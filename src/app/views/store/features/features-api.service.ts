...
export class FeaturesApiService {
  ...
  BLOG_LIST() {
    ...
    return this.http.get<any>(environment.ws_url+'/store/blog', httpOptions);
  }
  BLOG_DETAILS(x) {
    let httpOptions = { headers: new HttpHeaders({ 'Content-Type': 'application/json', 'Authorization': 'Bearer '+localStorage.getItem('store_token') }) };
    return this.http.get<any>(environment.ws_url+'/store/blog_details?blog_id='+x, httpOptions);
  }
  ADD_BLOG(x) {
    let httpOptions = { headers: new HttpHeaders({ 'Content-Type': 'application/json', 'Authorization': 'Bearer '+localStorage.getItem('store_token') }) };
